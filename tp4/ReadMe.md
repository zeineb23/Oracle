...
```sqlCREATE ROLE DevTeam;
GRANT CREATE PROCEDURE, CREATE VIEW, CREATE SEQUENCE, 
```
```sql
REVOKE SELECT ANY TABLE FROM TestTeam;
GRANT SELECT ON emp TO TestTeam;
```
```sqlCREATE SESSION, CREATE ANY TABLE, SELECT ANY TABLE, ALTER ANY TABLE, DROP ANY TABLE TO DevTeam;
```
```sqlCREATE ROLE TestTeam;
GRANT CONNECT, CREATE SESSION, SELECT ANY TABLE TO TestTeam;
```
